function StopwatchWidget({ currentTime, isRunning, lapTimes, onStart, onStop, onReset, onLap }) {
  return (
    <div className="flex flex-col items-center gap-2 w-full px-2">
      ...
      <span className={`text-2xl font-bold leading-none ${isRunning ? 'text-green-400' : 'text-white'}`}>
        {currentTime}
      </span>
      <div className="flex gap-1 flex-wrap justify-center max-h-10 overflow-y-auto w-full text-[7px]">
        {lapTimes.length > 0 ? (
          lapTimes.map((lap, index) => (
            <div key={index} className="bg-gray-800 rounded px-1.5 py-0.5 text-gray-300 whitespace-nowrap">
              L{index + 1} {lap}
            </div>
          ))
        ) : (
